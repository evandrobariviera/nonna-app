...
**Novos (2026-09-15), disparando de verdade:**
- `credenciais_solicitadas` — quando o time pede pro cliente preencher senhas/acessos (`ClientCredentialRequestController::store()`, botão "Solicitar ao cliente" na aba Senhas). `{{link_credenciais}}` aponta pra um link público tokenizado (`ClientCredentialRequest`, tabela `client_credential_requests`) onde o próprio contato preenche as plataformas, podendo voltar depois pra completar (não expira no primeiro envio, só por tempo — 15 dias). Ao enviar, cria linhas em `client_credentials` (com `credential_request_id` preenchido, pra distinguir do cadastro manual do time), marca `acessos_coletados` no Onboarding e avisa quem pediu via sino (`SystemNotificationService::send('credentials.recebidas', ...)`).
- `portal_definir_senha` — quando o time habilita acesso ao Portal em modo "link" (padrão, `ClientPortalAccessController::store()`) pra um contato sem senha ainda. `{{link_definir_senha}}` aponta pra um link público tokenizado (`PortalPasswordSetupToken`, tabela `portal_password_setup_tokens`, expira em 7 dias) onde o próprio contato cria a senha — ninguém do time chega a saber/definir a senha do cliente. Ao concluir, avisa quem gerou o link via sino (`SystemNotificationService::send('portal.senha_definida', ...)`). O modo "manual" (time digita a senha na hora) continua existindo como alternativa, disparando `portal_acesso_liberado` como sempre.
