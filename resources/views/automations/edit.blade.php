                ...
                <div class="grid gap-4">
                    ...
                    <div x-show="actionType === 'structure_ata'" x-cloak class="grid gap-3">
                        <p class="text-sm" style="color:var(--muted)">
                            ...
                            ATA a partir dela (atualiza a própria ATA da reunião, sem notificar ninguém — use
                    ...
                    <div x-show="actionType === 'adjust_date'" x-cloak class="grid gap-3">
                        ...
                        <div>
                            ...
                            <select name="action_config[date_base]" x-model="adjustDateBase"
                                    class="w-full px-3 py-2.5 text-sm focus:outline-none"
                                    style="background:var(--s3); border:1px solid var(--border); border-radius:8px; color:var(--text)">
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            <div>
                                <label class="block text-xs font-semibold mb-1" style="color:var(--muted); letter-spacing:.05em">QUAL DATA *</label>
                                <select name="action_config[date_field]" x-model="adjustDateField"
                                        x-init="$nextTick(() => $el.value = adjustDateField)"
                                        class="w-full px-3 py-2.5 text-sm focus:outline-none"
                                        style="background:var(--s3); border:1px solid var(--border); border-radius:8px; color:var(--text)">
                                    <option value="">Selecione...</option>
                                    <template x-for="[val, label] in Object.entries(dateFieldsMap[entityType] || {})" :key="val">
                                        <option :value="val" x-text="label" :selected="val === adjustDateField"></option>
                                    </template>
                                </select>
                            </div>
                            <div>
                                <label class="block text-xs font-semibold mb-1" style="color:var(--muted); letter-spacing:.05em">DESLOCAR (DIAS) *</label>
                                <input type="number" name="action_config[offset_days]"
                                       value="{{ old('action_config.offset_days', $automation->action_config['offset_days'] ?? 0) }}"
                        ...
                        <label class="flex items-center gap-2 cursor-pointer text-xs" style="color:var(--text)">
                            ...
                            <input type="checkbox" name="action_config[skip_weekends]" value="1"
                                   {{ !empty($automation->action_config['skip_weekends']) ? 'checked' : '' }}
                                   style="width:16px; height:16px; accent-color:var(--purple)">
                            Rolar pro próximo dia útil se cair em fim de semana
                        </label>
                    </div>
...
function automationBuilder() {
    return {
        ...
        adjustDateField: '{{ old('action_config.date_field', $automation->action_config['date_field'] ?? '') }}',
        adjustDateBase: '{{ old('action_config.date_base', $automation->action_config['date_base'] ?? 'field') }}',
