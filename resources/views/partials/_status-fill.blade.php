{{-- Botão de preenchimento colorido + dropdown de Status (padrão "Monday").
     ...
     background através do menu único compartilhado ($store.badgeFill, ver
     resources/js/monday-fill.js e components/badge-fill-menu.blade.php — precisa
     de um <x-badge-fill-menu /> em algum lugar da página, fora da área trocada
     por live-filter), sem recarregar a página; espera reativo no x-data do
     ancestral: statusKey/statusLabel/statusColor (ver _task-tr.blade.php). Fica
     desligado por padrão pra não mexer no uso na Central de Aprovações
     (showCurrent=false), que não precisa disso.
     Precisa de um ancestral com x-data contendo statusOpen/statusStyle (modo não-ajax)
     e de um container (<td>/<div>) com position:relative + tamanho definido —
     funciona em tabela ou fora dela, o botão preenche 100% do pai. --}}
@php
...
@if($ajax)
    <button @click="$store.badgeFill.openFor($el, { field: 'status', taskId: '{{ $task->id }}', url: '{{ $statusUrl }}', currentKey: statusKey })" type="button"
