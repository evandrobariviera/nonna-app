<x-app-layout>
    ...
    <div class="grid gap-3 mb-6" style="grid-template-columns: repeat(4, 1fr)">
        <div class="card px-4 py-4 text-left">
            <div class="text-2xl font-black mb-1" style="color:var(--text)">
                R$ {{ number_format($stats['month_spend'], 2, ',', '.') }}
            </div>
            <div class="text-xs font-mono uppercase tracking-widest" style="color:var(--muted)">Gasto no mês</div>
        </div>
        ...
        <div class="card px-4 py-4 text-left">
            ...
            <div class="text-xs font-mono uppercase tracking-widest" style="color:var(--muted)">Orçamento combinado</div>
        </div>
        ...
        <div class="card px-4 py-4 text-left">
            ...
            <div class="text-xs font-mono uppercase tracking-widest" style="color:var(--muted)">Clientes acima do orçado</div>
        </div>
    ...
    {{-- ── INSIGHTS ABERTOS ── --}}
    <div class="mb-6">
        <h3 class="text-xs font-mono uppercase tracking-widest mb-3" style="color:var(--muted)">
            Insights ({{ $openInsights->count() }})
        </h3>

        @if($openInsights->isEmpty())
            <div class="card px-6 py-8 text-center" style="color:var(--muted)">
                <p class="text-sm">Nenhum insight em aberto no momento.</p>
            </div>
        @else
    ...
    <form method="GET" action="{{ route('campaigns.index') }}" class="flex flex-wrap items-center gap-3 mb-5">
        ...
        <select name="platform"
            ...
            <option value="meta" {{ request('platform') === 'meta' ? 'selected' : '' }}>Meta Ads</option>
        ...
        @if(request()->hasAny(['client_id', 'platform', 'status']))
