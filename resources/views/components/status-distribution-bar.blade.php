{{--
    ...
    largura proporcional à contagem, cor = Lei das Cores. Panorama do fluxo (Backlog →
    Em Produção → Revisão → Ajuste → Aprovação → Despacho → Concluído) numa olhada só,
    em vez de só "% concluído". Reaproveitado no Dashboard e na tela de Sprint.
--}}
@props(['counts', 'total'])
...
<div {{ $attributes->class(['w-full h-2 rounded-full overflow-hidden flex']) }} style="background:var(--border2)">
    @foreach(\App\Models\Task::$statuses as $statusKey => $meta)
        ...
        <div class="h-2 transition-all" data-status-segment="{{ $statusKey }}"
             ...
             title="{{ $meta['label'] }}: {{ $cnt }} ({{ round($pct) }}%)"></div>
    @endforeach
</div>
